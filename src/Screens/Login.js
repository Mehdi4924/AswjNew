...
const Login = ({ navigation }) => {
            ...
            (data) => {
              ...
              const responseInfoCallback = (error, result) => {
                if (error) {
                  ...
                  // let Sign= auth().signInWithCredential(facebookCredential);
                  // console.log(Sign);
                  firebase.auth().signInWithCredential(facebookCredential).catch(function (error) {
                    ...
                    if (errorCode === 'auth/account-exists-with-different-credential') {
                      ...
                      setshowModal(true);
                      setMessage('Error:Email already associated with another account.');
  ...
  const __doSingIn = async (email, password) => {
    try {
      ...
      if (e.message == "[auth/wrong-password] The password is invalid or the user does not have a password.") {
        ...
        setshowModal(true);
        setMessage('Error: The password is invalid or the user does not have a password');
      }
        ...
        setshowModal(true);
        setMessage('Error: There is no user record corresponding to this identifier. The user may have been deleted.');
      }
        ...
        setshowModal(true);
        setMessage('Error:We have blocked all requests from this device due to unusual activity.');
      }
    }
  };
 ...
 <View>
        ...
        >
          ...
          >
            <Text style={[style.thickHeader, { letterSpacing: 2 }]}>Forgot</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => navigation.navigate("SignUp")}>
            <Text style={[style.thickHeader, { letterSpacing: 2 }]}>Register</Text>
          </TouchableOpacity>
        </View>
        ...
        >
          ...
          <Text
            style={[style.thickHeader, { fontFamily: 'Montserrat-Medium', fontSize: 13, letterSpacing: 1 }]}
          >
            LOGIN WITH FACEBOOK
          </Text>
        ...
        >
          ...
          <Text
            style={[style.thickHeader, { fontFamily: 'Montserrat-Bold', fontSize: 13, letterSpacing: 0 ,color:'#000'}]}
          >
Sign In With Apple
          </Text>
